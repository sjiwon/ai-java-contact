...
import com.sjiwon.contact.application.ConsoleRdbProcessor;
...
public class MainController {
    ...
    private final ConsoleRdbProcessor consoleRdbProcessor;
    ...
    private void show() {
        final List<Contact> contacts = consoleRdbProcessor.findAll();
    ...
    private void validatePhoneIsExists(final String value) {
        if (consoleRdbProcessor.isPhoneUsedByOther(value)) {
    ...
    private void register(final Contact contact) {
        consoleRdbProcessor.create(contact);
    }
    ...
    private void proceedSearch(final SearchType type, final String value) {
        ...
        final List<Contact> contacts = consoleRdbProcessor.findByCondition(type, value);
        System.out.println(organizeContact(contacts));
    }
    ...
    private void delete() {
        while (true) {
            try {
                ...
                consoleRdbProcessor.delete(contacts.get(row - 1));
